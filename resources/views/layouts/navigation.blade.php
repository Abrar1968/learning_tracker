        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="transition transform hover:scale-105">
                        <x-application-logo class="block h-9 w-auto text-indigo-600 fill-current" />
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')"
                        class="text-slate-600 hover:text-indigo-600 active:text-indigo-700 font-medium">
                        {{ __('Dashboard') }}
                    </x-nav-link>
                    <x-nav-link :href="route('roadmaps.index')" :active="request()->routeIs('roadmaps.*')"
                        class="text-slate-600 hover:text-indigo-600 active:text-indigo-700 font-medium">
                        {{ __('Roadmaps') }}
                    </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-slate-600 bg-transparent hover:text-indigo-600 focus:outline-none transition ease-in-out duration-150">
